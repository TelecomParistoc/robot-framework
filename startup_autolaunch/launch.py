...
if not os.isdir('/var/robots_logs'):
    os.makedirs('/var/robots_logs')

logfile = datetime.now().strftime('/var/robots_logs/robot_%H_%M_%d_%m_%Y.log')
program_to_launch_path = content[0]
command_path = content[1]
program_path = content[2]
...
if sys.argv[1] == "start":
    ...
    system("killall "+program_path.split('/')[-1])
    print("Robot is dead")
else:
    print("Usage: /etc/init.d/robot_auto {start|stop}")
